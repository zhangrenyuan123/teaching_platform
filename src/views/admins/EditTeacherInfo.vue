        ...
        <section>
            <el-form :model="info" :rules="registerRules" ref="registerRef" 
                ...
                <el-form-item label="教师号" prop="teaNum">
                    <el-input v-model="info.teaNum"></el-input>
                </el-form-item>
...
export default {
    ...
    methods: {
        ...
        async getTeacherInfo(){
            let {data:result} = await this.$http.post('/getTeacherInfo',{
                id:this.id
            });
            if(result.meta.status === 200){
                this.info.teaNum = result.data.Teacher_num;
                this.info.name = result.data.Teacher_name;
                this.info.pwd = result.data.Teacher_pwd;
                this.info.sex = result.data.Teacher_sex;
                this.info.level = result.data.Teacher_level;
                this.info.tel = result.data.Teacher_tel;
                this.info.email = result.data.Teacher_email;
                this.info.qq = result.data.Teacher_qq;
            }
        },
        save(){
            this.$refs.registerRef.validate(async valid => {
                if(!valid) return;
                let {data : result} = await this.$http.post('/editTeacherInfo',{
                    ...this.info,
                    id:this.id
                });
                if(result.meta.status === 200){
